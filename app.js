...
// LEGGER TIL ALLE STEDENE I SELECT-LISTA
...
function searchElementClicked(number){
    console.log(cities[number].city);
...
async function updateAll(place){
        ...
        for(i = 0; i < 7; i++){
            ...
            if(sc[nextDays[i]].data.instant.details.air_temperature > highestTemp){
                highestTemp = sc[nextDays[i]].data.instant.details.air_temperature;
            }
        }
        document.getElementById("cell-day1").innerHTML += ` (nå)`;
        for(i = 0; i < 7; i++){
            graph_grid.innerHTML += `<div class="graph-col" style="height:${sc[nextDays[i]].data.instant.details.air_temperature * (220 / highestTemp)}px; margin-top: ${220 - (sc[nextDays[i]].data.instant.details.air_temperature * (220 / highestTemp))}px">
                ${sc[nextDays[i]].data.instant.details.air_temperature}° ${data.properties.meta.units.air_temperature}
            </div>`; 
        }
        ...
        graph_lines.style.height = graph_grid.height;
...
search_input.oninput = function(){
    ...
    for(i = 0 ; i <= cities.length - 1; i++){
        ...
        if(search >= 0){
            ...
            search_results.innerHTML += `<div onClick="searchElementClicked(${i})">${cities[i].city}</div>`;
        }
    ...
    if(numberOfSearchResults == 0){
        search_results.innerHTML = `<div>Stedet "${search_input.value}" finnes ikke i våre databaser</div>`;
    }
